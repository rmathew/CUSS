...
// Translated bitmap-data of "VGA-ROM.F16" from the font-collection by
// Joseph (Yossi) Gil that used to be available at:
//   ftp://ftp.simtel.net/pub/simtelnet/msdos/screen/fntcol16.zip
static uint8_t vga_8x16[] = {
    // 0x00
...
static inline bool CreateFontSurface(const SDL_PixelFormat* restrict
  pixel_format, CuError* restrict err) {
#define FONT_SURFACE_DEPTH 32
    SDL_Surface* tmp_surface = SDL_CreateRGBSurfaceWithFormat(/*flags=*/0,
      /*width=*/VGA_FONT_CHARS * VGA_FONT_WIDTH, /*height=*/VGA_FONT_HEIGHT,
      FONT_SURFACE_DEPTH, SDL_PIXELFORMAT_ARGB8888);
    if (tmp_surface == NULL) {
        return CuErrMsg(err,
          "Unable to create temporary font-surface: %s", SDL_GetError());
        return false;
    }
    SDL_SetColorKey(tmp_surface, SDL_TRUE, 0x00000000U);

    const int bpp = FONT_SURFACE_DEPTH / 8;
    uint8_t* row_pos = NULL;
    for (size_t i = 0U; i < sizeof vga_8x16; i++) {
        // Reset `row_pos` if we are starting to render a new character.
        if ((i % VGA_FONT_HEIGHT) == 0) {
            row_pos = (uint8_t*)tmp_surface->pixels +
              ((i / VGA_FONT_HEIGHT) * VGA_FONT_WIDTH * bpp);
        } else {
            row_pos += tmp_surface->pitch;
        }
        const uint8_t row_bits = vga_8x16[i];
        uint32_t* col_pos = (uint32_t*)row_pos;
        for (size_t j = 0U; j < VGA_FONT_WIDTH; j++) {
            const int col_bit = VGA_FONT_WIDTH - j - 1;
            if (row_bits & (0x01 << col_bit)) {
                *col_pos = 0xFFFFFFFFU;
            } else {
                *col_pos = 0x00000000U;
            }
...
#undef FONT_SURFACE_DEPTH

    font_surface = SDL_ConvertSurfaceFormat(tmp_surface, pixel_format->format,
      /*flags=*/0);
    if (font_surface == NULL) {
        return CuErrMsg(err, "Unable to covert font-surface: %s",
          SDL_GetError());
    }
...
bool CuSdlTxtSetUp(const SDL_PixelFormat* restrict pixel_format,
  CuError* restrict err) {
    // Sanity-check of the size of the bitmap font-data.
    if ((sizeof vga_8x16) != (VGA_FONT_CHARS * VGA_FONT_HEIGHT)) {
        return CuErrMsg(err, "Unexpected size of vga_8x16[]: %u (vs %u).",
          sizeof vga_8x16, VGA_FONT_CHARS * VGA_FONT_HEIGHT);
        return false;
    }
...
int CuSdlTxtHeight() { return VGA_FONT_HEIGHT; }

bool CuSdlTxtSetColor(const SDL_Color* restrict clr, CuError* restrict err) {
    if (SDL_SetSurfaceColorMod(font_surface, clr->r, clr->g, clr->b) != 0) {
        return CuErrMsg(err, "Unable to set font-surface color-modulation: %s",
          SDL_GetError());
    }
    if (SDL_SetSurfaceAlphaMod(font_surface, clr->a) != 0) {
        return CuErrMsg(err, "Unable to set font-surface alpha-modulation: %s",
          SDL_GetError());
    }
    return true;
}
...
static inline bool RenderByte(SDL_Surface* restrict screen,
  const SDL_Point* restrict pos, uint8_t byt, CuError* restrict err) {
    SDL_Rect src = {.x = (int)byt * VGA_FONT_WIDTH, .y = 0, .w = VGA_FONT_WIDTH,
        .h = VGA_FONT_HEIGHT};
    SDL_Rect dst = {.x = pos->x, .y = pos->y, .w = VGA_FONT_WIDTH,
        .h = VGA_FONT_HEIGHT};
    if (SDL_BlitSurface(font_surface, &src, screen, &dst) != 0) {
        return CuErrMsg(err, "Unable to render byte: %s", SDL_GetError());
    }
    return true;
}
